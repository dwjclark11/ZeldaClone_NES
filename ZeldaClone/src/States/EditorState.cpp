...
EditorState::EditorState()
	...
	, m_InputManager{ InputManager::GetInstance() }, m_Registry{ Registry::Instance() }
	, m_Loader{ *m_pMouseControl }, m_bEditor{ false }, m_bKeyDown{ false }
{
...
void EditorState::Render()
{
	...
	for (int i = 0; i < yNum; i++)
	{
		...
		{
			SDL_Rect newRect{
				(j * gridSize) - camera.GetCameraPos().x,
				(i * gridSize) - camera.GetCameraPos().y
				, 64, 64
			};
	...
	m_pMouseControl->Update(
		m_Game.GetAssetManager(), m_Game.GetRenderer(),
		m_Game.GetMouseBox(), m_Game.GetEvent(), m_Game.GetCamera().GetCameraRect()
	);

	Registry::Instance().GetSystem<RenderCollisionSystem>().Update(
		m_Game.GetRenderer(), m_Game.GetCamera().GetCameraRect()
	);

	Registry::Instance().GetSystem<RenderEditorGUISystem>().Update(
		m_Game.GetAssetManager(), m_Game.GetRenderer()
	);
