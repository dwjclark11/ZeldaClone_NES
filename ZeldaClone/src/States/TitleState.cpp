...
void TitleState::AnimateTitle()
{
	if (m_pCamMoveTimer->GetTicks() > 10000 && m_pCamMoveTimer->GetTicks() < 15000)
	{
		...
		if (animation.currentFrame == 8 && !bOffset)
		{
...
TitleState::TitleState()
	...
	, m_Timer{ 0 }
	, m_bScroll{ false }, m_bScrollFinished{ false }, bOffset{ false }
...
{
	AnimateTitle();
	m_Registry.GetSystem<RenderTitleSystem>().Update(m_Game.GetRenderer(), m_Game.GetAssetManager(), m_Game.GetCamera());
}
...
bool TitleState::OnEnter()
{
	...
	if (!m_Game.GetAssetManager()->HasTexture("TitleScreen")) 
		m_Game.GetAssetManager()->AddTextures(m_Game.GetRenderer(), "TitleScreen", "./Assets/Backgrounds/DemoZeldaTitleScreen.png");
	if (!m_Game.GetAssetManager()->HasTexture("title_scroll")) 
		m_Game.GetAssetManager()->AddTextures(m_Game.GetRenderer(), "title_scroll", "./Assets/Backgrounds/titleScroll.png");
	if (!m_Game.GetAssetManager()->HasTexture("waterfall")) 
		m_Game.GetAssetManager()->AddTextures(m_Game.GetRenderer(), "waterfall", "./Assets/Backgrounds/waterfall.png");
