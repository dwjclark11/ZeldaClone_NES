...
public:
	...
	{
		EMPTY = 0, BOOMERANG, MAGIC_BOOMERANG, BOMB, CANDLE, 
		MAGIC_CANDLE, BOW, WOOD_ARROWS, MAGIC_ARROWS, FOOD, 
		WHISTLE, MAGIC_ROD, FLUTE, RAFT, LADDER, 
		BLUE_RING, RED_RING, SHIELD, MAGIC_SHIELD, MAP, 
		POWER_BRACLET, MASTER_KEY, BLUE_POTION, RED_POTION,
		WOOD_SWORD, SILVER_SWORD, MAGIC_SWORD
	};
	enum class ItemType
	{
		EMPTY = 0,BOOMERANG, BOMB, POTION_BLUE, POTION_RED,
		CANDLE, BOW, FOOD, WHISTLE, MAGIC_ROD
	};
...
public:
	...
	void UseKey() { if (m_TotalKeys > 0) m_TotalKeys--; }
	void UseBomb() { if (m_TotalBombs > 0) m_TotalBombs--; }
	void UseArrow() { if (m_TotalRupees > 0) m_TotalRupees--; }
